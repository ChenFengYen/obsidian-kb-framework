...
def install_conventions(cfg, target):
    ...
    lines.extend([
        ...
        '1. Read the index first - it is short enough that you never have to guess',
        ...
        '   ```', '',
        '   `--index` gives one line per rule: id, triggers, name, one-line version.',
        '   `--for-trigger` narrows to a term from the closed vocabulary in [[registry]]',
        '   once the task is clear. Then follow the links inside a rule - a rule names',
        '   its neighbours, and the neighbour is often the one that applies.',
